...
export const generateCohortData = () => {
  ...
  for (let i = 0; i < months.length; i++) {
    const signups = Math.floor(Math.random() * 500) + 900;
    const day7Retention = Math.floor(Math.random() * 20) + 60;
    const day30Retention = Math.floor(Math.random() * 15) + 35;
    const day60Retention = Math.floor(Math.random() * 10) + 25;
    const invitesSent = Math.floor(Math.random() * 150) + 350;
    const invitesAccepted = Math.floor(
      invitesSent * (Math.random() * 0.3 + 0.6)
    );
    const trialStarted = Math.floor(
      invitesAccepted * (Math.random() * 0.2 + 0.7)
    );
    const paidConversions = Math.floor(
      trialStarted * (Math.random() * 0.2 + 0.5)
    );
    const topState = states[Math.floor(Math.random() * states.length)];
